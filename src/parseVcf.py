...
class multiMixtureReader():
        ...
        def __init__(self, sampleIds, persistenceDir, summaryStore=None):
                ...
                ## check that result store is of the correct class etc
        ...
        def summarise_selection(self, selection, selectionDescription=None):
                ...
                for sampleId in self.mixtureReader.keys():
                        self.mixtureReader[sampleId].summarise_selection(selection, selectionDescription=selectionDescription)
                self._aggregate()
                
        def summarise_byFilter(self, minDepth, minP, selectionDescription=None):
                ...
                """
                for sampleId in self.mixtureReader.keys():
                        self.mixtureReader[sampleId].summarise_byFilter(minDepth, minP, selectionDescription=selectionDescription)
                self._aggregate()               
class mixtureReader():
        """ a class reading the results of mixture computations from one TB sequence, as generated by vcfSQL """
        def __init__(self, sampleId, persistenceDir, summaryStore=None):
        ...
        def _generateSelectionDescription(self, selection=None, minDepth=None, minP=None):
                ...
                parameters={'selection':selection, 'minDepth':minDepth,'minP':minP}
        ...
        def summarise_selection(self, selection, selectionDescription=None):
                ...
                if self.summaryStore is None:         # then it has to compute the summary ab initio
                        self.summarise_selection_fromdb(selection)
                else:
                   ...
                   if selectionDescription is None:
                        ...
                        selDes=self._generateSelectionDescription(selection=selection, minP=None, minDepth=None)
                
                        # attempt to recover the data from the store; if not present, add it.
                        
                        
        def summarise_byFilter(self, minDepth, minP, summaryStore=None, selectionDescription=None):
                ...
                if self.summaryStore is None:         # then it has to compute the summary ab initio
                   ...
                   if selectionDescription is None:
                        # then we must compute a selectionDescription from the criteria passed to us.
                        selDes=self._generateSelectionDescription(selection=None, minP=minP, minDepth=minDepth)
                        # attempt to recover the data from the store; if not present, add it.                
...
class test_mixtureReader_generateSelectionDescription_1(unittest.TestCase):
    def runTest(self):
        ...
        with self.assertRaises(ValueError):     
                mr._generateSelectionDescription(selection=None, minP=None, minDepth=None)
class test_mixtureReader_generateSelectionDescription_2(unittest.TestCase):
    def runTest(self):
        """ tests the generation of a SelectionDescription using a hash of the values passed to it """
        targetdir=os.path.join('..','unitTest_tmp')    
        mr=mixtureReader(sampleId='guid1',persistenceDir=targetdir)
        res1=mr._generateSelectionDescription(selection=set([1,2,3]), minP=None, minDepth=None)          # passed a set
        self.assertEqual(res1,'8f91e12d8c2f693a91ccbe8bc707af1c')
        res2=mr._generateSelectionDescription(selection=[1,2,3], minP=None, minDepth=None)               # passed a list
        self.assertEqual(res2,'8f91e12d8c2f693a91ccbe8bc707af1c')
        res3=mr._generateSelectionDescription(selection=[3,2,1], minP=None, minDepth=None)               # passed a list; check order
        self.assertEqual(res3,'8f91e12d8c2f693a91ccbe8bc707af1c')                
        res4=mr._generateSelectionDescription(selection=[3,2,1], minP=5, minDepth=None)                  # does minP matter
        self.assertEqual(res4,'525b658a458ceff4dcb6b3242d4e7fe9')
        res5=mr._generateSelectionDescription(selection=[3,2,1], minP=None, minDepth=5)                  # does minDepth matter
        self.assertEqual(res5,'eeb15dabd99db9dc7c778eb3aed10b7d')   